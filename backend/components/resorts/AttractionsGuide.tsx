  ...
  return (
    <section className="py-12">
      <div className="flex items-center gap-3 mb-8">
        ...
        <h2 className="text-2xl font-bold font-serif text-navy-950">Nearby Attractions</h2>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        {attractions.map((attraction, i) => (
          <motion.div
            ...
            className="flex gap-4 bg-sand-50 rounded-2xl p-5 border border-sand-100 hover:border-gold-200 transition-colors group"
          >
            {/* Icon */}
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-start justify-between gap-2 mb-1">
                <h4 className="font-bold text-navy-950 text-base leading-snug">{attraction.name}</h4>
                <div className="flex items-center gap-1 flex-shrink-0 text-navy-950/50">
                  <Footprints className="w-3.5 h-3.5" />
                  <span className="text-xs font-semibold">{attraction.distanceKm} km</span>
                </div>
              </div>
              <p className="text-sm text-navy-950/60 leading-relaxed">{attraction.description}</p>
            </div>
          </motion.div>
        ))}
      </div>
